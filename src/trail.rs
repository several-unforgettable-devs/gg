...
fn build_quat_to_align_vec(
    original_vector : Vec3,
    new_direction : Vec3
) -> Quat {
...
fn calc_trail_spawn_point(
    ...
    let projected_dir = if valid_vec3(&player_velocity_unit) { player_velocity_unit } else { Vec3::zero() };
    ...
    let final_pos = player_position + projected_dir * MAX_TRAIL_DIST * 0.75
